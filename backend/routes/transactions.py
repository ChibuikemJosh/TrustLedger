...
@router.post("/log")
async def log_manual_transaction(tx_data: TransactionRequest, current_user: dict = Depends(get_current_firebase_user)):
...
async def update_transaction(tx_id: str, new_data: TransactionRequest, current_user: dict = Depends(get_current_firebase_user)):
    """Modifies an unverified transaction. Rejects any mutations if locked via background OPay webhooks."""
    user_id = current_user.get("uid") or current_user.get("user_id")
    
    # 1. Enforce strict lock rules on verified records
...
async def fetch_user_dashboard_payload(current_user: dict = Depends(get_current_firebase_user)):
